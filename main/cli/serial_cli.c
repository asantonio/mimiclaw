...
static int cmd_set_ollama_url(int argc, char **argv)
{
    ...
    esp_err_t err = llm_set_ollama_url(ollama_url_args.url->sval[0]);
    if (err == ESP_OK) {
...
esp_err_t serial_cli_init(void)
{
    ...
    /* set_ollama_url */
    ollama_url_args.url = arg_str1(NULL, NULL, "<url>", "Ollama base URL (e.g. http://host:11434/v1/chat/completions)");
    ollama_url_args.end = arg_end(1);
    esp_console_cmd_t ollama_url_cmd = {
        .command = "set_ollama_url",
        .help = "Override the Ollama base URL (persisted to NVS)",
